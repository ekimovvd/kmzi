<template>
  <div class="gronsfeld">
    <div class="gronsfeld__block">
      <input
        class="gronsfeld__input"
        type="text"
    ...
    handleEncode() {
      ...
      encodeArray.forEach((symbol, symbolIndex) => {
        if (this.arr_ru.includes(symbol)) {
          let key = parseInt(keyArray[symbolIndex]);
          ...
          if (shift < 0) {
            encode += this.arr_ru[arrayLength + shift];
          } else {
          ...
          let shift = this.arr_RU.indexOf(symbol) + parseInt(key);
          let arrayLength = this.arr_RU.length;
          ...
          let shift = this.arr_en.indexOf(symbol) + parseInt(key);
          let arrayLength = this.arr_en.length;

          if (shift < 0) {
            encode += this.arr_en[arrayLength + shift];
          } else {
